...
1.  If you haven't yet, convert the current camera AdLib to AdLib Actions by exporting the `IBlueprintActionManifest` as part of your `getRundown` implementation and implementing an AdLib Action in your `executeAction` handler that adds your camera piece.
